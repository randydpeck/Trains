...
#ifndef NETWORK_TST_h
#define NETWORK_TST_h

#include "Network_RS485.h"
#include "Train_Consts_Global.h"

class Network_TST : public Network_RS485 {

  public:

    // Constructor:
    Network_TST(HardwareSerial * hdwrSerial, long unsigned int baud, Display_2004 * LCD2004);
  ...
  private:
    ...
    void SetTurnoutButtonNum(const byte button);  // Inserts button number into the appropriate byte
    // A_BTN (private): Inserts byte button number pressed into the outgoing message byte array


    byte GetTurnoutButtonNum();
