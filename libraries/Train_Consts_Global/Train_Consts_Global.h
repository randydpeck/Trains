...
// constants.h for global constants
// globals.h for global variable declarations
// globals.cpp for global variable definitions
...
// will be many commands between Arduinos, even though most may not be for THIS Arduino.  If the buffer overflows,
// then we will be totally screwed up (but it will be apparent in the checksum.)
// extern const byte RS485_MAX_LEN;       // buffer length to hold the longest possible RS485 message.  Just a guess.
extern       byte RS485MsgIncoming[];  // No need to initialize contents
extern       byte RS485MsgOutgoing[];
extern const byte RS485_LEN_OFFSET;    // first byte of message is always total message length in bytes
extern const byte RS485_TO_OFFSET;     // second byte of message is the ID of the Arduino the message is addressed to
extern const byte RS485_FROM_OFFSET;   // third byte of message is the ID of the Arduino the message is coming from
// Note also that the LAST byte of the message is a CRC8 checksum of all bytes except the last
extern const byte RS485_TRANSMIT;      // HIGH = 0x1.  How to set TX_CONTROL pin when we want to transmit RS485
extern const byte RS485_RECEIVE;       // LOW = 0x0.  How to set TX_CONTROL pin when we want to receive (or NOT transmit) RS485
