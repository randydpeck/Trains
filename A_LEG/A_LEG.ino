...
void populateDelayedActionTable() {
  ...
  for (unsigned int delayedActionRec = 0; delayedActionRec < delayedActionTestRecs; delayedActionRec++) {  // Using offset 0 for ease of calcs
    ...
    // WHAT IS THE PROBLEM WITH THE = SIGN HERE????  RED DOT AT RIGHT, BUT NO ERROR ON COMPILE.  MAYBE THIS NEEDS TO BE A memcpy???
    // actionElement = actionArray[delayedActionRec];
    memcpy(&actionElement, &actionArray[delayedActionRec], FRAM2_ACTION_LEN);  // Need to test to see if this works as expected...
    writeActionElement();
  }
  return;
}
